        ...
        {list.length === 0 ? (
          <div className="empty">
            ...
            <div className="empty-sub">
              {filter === 'active' ? 'Keine offenen Verleihungen.' : 'Trage deine erste Verleihung ein.'}
            </div>
          </div>
        ) : (
          <div className="card">
            {list.map(loan => {
              ...
              const overdue = !loan.isReturned && loan.dueDate && new Date(loan.dueDate) < new Date()

              return (
                <SwipeRow key={loan.id} actions={[
                  !loan.isReturned && { icon: '✓', label: 'Zurück', color: '#00C853', onClick: () => s.markReturned(loan.id) },
                  { icon: '🗑', label: 'Löschen', color: '#FF3B30', onClick: () => {
                    if (confirm('Verleihung löschen?')) s.deleteLoan(loan.id)
                  }}
                ].filter(Boolean)}>
                  <div className="card-row">
                    ...
                    <div className="row-main">
                      ...
                      <div className="row-sub">{person?.name ?? '–'} · {fmtDate(loan.lentOn)}</div>
                      {loan.dueDate && !loan.isReturned && (
                        <div className="row-sub" style={{ color: overdue ? 'var(--red)' : undefined }}>
                          {overdue ? '⚠ ' : ''}Fällig: {fmtDate(loan.dueDate)}
                        </div>
                      )}
                    </div>
                    <div>
